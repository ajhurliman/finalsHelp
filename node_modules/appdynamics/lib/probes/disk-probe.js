...
DiskProbe.prototype.attach = function(obj) {
  ...
  self.reads = self.agent.metricsManager.createMetric('Disk', 'Data read per sec', 'KB', 'sum');
  self.writes = self.agent.metricsManager.createMetric('Disk', 'Data written per sec', 'KB', 'sum');
  ...
  proxy.before(obj, 'read', function(obj, args, ret) {
    var fd = args[0];
    if (!fileFDs[fd]) return;
    proxy.callback(args, -1, function(obj, args) {
      var count = args[1] || 0;
  ...
  proxy.before(obj, 'write', function(obj, args, ret) {
    var fd = args[0];
    if (!fileFDs[fd]) return;
    proxy.callback(args, -1, function(obj, args) {
      var count = args[1] || 0;
