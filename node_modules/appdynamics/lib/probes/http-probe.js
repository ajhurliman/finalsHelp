...
function createBTCallback(proxy, profiler, time, transaction, req, res, thread, callback, self, args) {
    var didRun = false;
    ...
    return function () {
        ...
        try {
            callback = proxy.wrapWithThreadProxyIfEnabled(callback);
...
HttpProbe.prototype.__createRequestHandler = function (callback, isHTTPs) {
    ...
    return function (req, res) {
        ...
        var delayedCallback = createBTCallback(self.agent.proxy,
                                               profiler,
                                               time,
                                               transaction,
                                               req, res,
...
HttpProbe.prototype.attach = function(obj, moduleName) {
  ...
  proxy.around(obj, 'request', function(obj, args, locals) {
    ...
    if (self.agent.backendConfig.isQueryStringRequired()) {
      var query = locals.opts.query || url.parse(path).query;
      supportedProperties['QUERY STRING'] = query;
    }
