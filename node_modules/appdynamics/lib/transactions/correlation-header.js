...
CorrelationHeader.prototype.parse = function(headerString) {
  ...
  if(accountGuid && accountGuid != self.correlation.accountGuid) {
    self.agent.logger.log("CorrelationHeader.parse: Remote account GUID [" + accountGuid + "] and local [" + self.correlation.accountGuid + "] do not match, not processing");
    return false;
  }

  if(controllerGuid && controllerGuid != self.correlation.controllerGuid) {
    self.agent.logger.log("CorrelationHeader.parse: Remote controller GUID [" + controllerGuid + "] and local [" + self.correlation.controllerGuid + "] do not match, not processing");
    return false;
  }
  ...
  if(appId && appId !== self.correlation.appId) {
    if(!(accountGuid && controllerGuid)) {
      self.agent.logger.log("CorrelationHeader.parse: Remote app ID [" + appId + "] and local app ID [" + self.correlation.appId + "] do not match, not processing");
      return false;
    }
    else {
  ...
  if(crossAppCorrelation) {
    ...
    if(m && m.length == 2) {
      ...
      if (crossAppCorrelationBackendId <= 0) {
        self.agent.logger.log("CorrelationHeader.parse: unresolved backend ID is invalid for cross app correlation: "+ lastComponentToId);
        return false;
      }
      self.agent.logger.log("CorrelationHeader.parse: crossAppCorrelationBackendId = " + crossAppCorrelationBackendId);
    ...
    if (crossAppCorrelationToAppId == self.correlation.appId) {
      self.agent.logger.log("CorrelationHeader.parse: foreign backend is already resolved to this application: " + lastComponentToId);
      self.crossAppCorrelation = true;
      return false;
    }
    ...
    if (crossAppCorrelationBackendId <= 0) {
      self.agent.logger.log("CorrelationHeader.parse: cross app correlation header is missing valid " +
        self.correlation.UNRESOLVED_EXIT_ID + " subheader:" + headerString);
      return false;
    }
    self.agent.logger.log("CorrelationHeader.parse: re-resolving foreign backend " +
  ...
  if(self.getSubHeader(self.correlation.UNRESOLVED_EXIT_ID) !== undefined) {
    ...
    if(unresolvedExitId > 0) {
      ...
      if (m && (m.length == 2)) {
        self.agent.logger.log("Re-resolving backend " + unresolvedExitId + " to this tier, it was resolve to an application.");
        self.agent.proxyTransport.sendSelfReResolution({
          backendId: unresolvedExitId
        });
