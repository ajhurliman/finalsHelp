...
function BackendConfig(agent) {
  ...
  this.orderedDbProperties = ['HOST', 'PORT', 'DATABASE', 'VENDOR', 'VERSION'];
...
BackendConfig.prototype.init = function() {
  ...
  self.agent.on('configUpdated', function() {
    ...
    self.queryStringRequired = false;
    if (self.httpBackendConfig) {
      self.httpBackendConfig.forEach(function(config) {
        setCustomConfigFlag(config, 'httpMatchRule');

        if (!self.queryStringRequired) {
          if (config.matchRule &&
              config.matchRule.httpMatchRule &&
              config.matchRule.httpMatchRule.queryString) {
            self.queryStringRequired = true;
          }
          else if (config.namingRule &&
                   config.namingRule.httpNamingRule &&
                   config.namingRule.httpNamingRule.queryString) {
            self.queryStringRequired = true;
          }
        }
      });
...
BackendConfig.prototype.isQueryStringRequired = function() {
  return this.queryStringRequired;
};
...
function getIdentifyingProperties(exprEval, namingRules, ruleType, supportedProperties) {
  var properties = {};
