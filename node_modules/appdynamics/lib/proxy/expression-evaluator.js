...
ExpressionEvaluator.prototype.evaluate = function(node, ctx) {
  switch (node.type) {
    ...
    case 'CALLER':
      return this.opCaller(node.callerOp, ctx);
    case 'STRINGMATCH':
...
ExpressionEvaluator.prototype.opGetter = function(op, ctx) {
  ...
  if (!base) {
    this.agent.logger.error('getter base is not an object');
    return undefined;
  }
...
ExpressionEvaluator.prototype.opCaller = function(op, ctx) {
  var base = this.evaluate(op.base, ctx);
  if (!base) {
    this.agent.logger.error('caller base is not an object');
    return undefined;
  }

  var params = [];
  if (op.params) {
    for (var i = 0, length = op.params.length; i < length; ++i) {
      params.push(this.evaluate(op.params[i], ctx));
    }
  }

  return base[op.method].apply(base, params);
};
...
ExpressionEvaluator.prototype.opSplit = function(op, ctx) {
  ...
  if (op.firstNSegments !== undefined) {
    result = result.slice(0, op.firstNSegments);
  }
  else if (op.lastNSegments !== undefined) {
    result = result.slice(-op.lastNSegments);
  }
  else if (op.specificSegments !== undefined) {
    var segments = [];
    for (var i = 0, length = op.specificSegments.length; i < length; ++i) {
      // segment number is 1 based
      var index = op.specificSegments[i];
      if (index > 0 && index <= result.length)
        segments.push(result[index - 1]);
    }
...
ExpressionEvaluator.prototype.entityValue = function(entity, ctx) {
  switch (entity.type) {
    ...
    default:
      this.agent.logger.error('unhandled entity value type: ' + entity.type);
  }
};

ExpressionEvaluator.prototype.entityString = function(entity, ctx) {
  return entity;
};

ExpressionEvaluator.prototype.entityInteger = function(entity, ctx) {
  return entity;
};
